...
const captureLogger = {
  ...
  info: (message, data) => {
    captureLogger.log(message, 'info', data);
  },
  warn: (message, data) => {
    captureLogger.log(message, 'warn', data);
  },
  error: (message, data) => {
    captureLogger.log(message, 'error', data);
  },
};
...
const initSettingsUI = () => {
  ...
  const populateSettingsUI = () => {
    ...
    $('#st-mod-ss-quality')
      .val(currentSettings.imageQuality)
      .prop('disabled', !['jpeg', 'webp'].includes(currentSettings.imageFormat));
  ...
  const bindEvents = () => {
    ...
    const backToMainPanel = () => {
      ...
      $(window)
        .off('resize.mss.help')
        .on('resize.mss.main', () => centerElement(panelElement));
    ...
    const openHelpPanel = () => {
      ...
      $(window)
        .off('resize.mss.main')
        .on('resize.mss.help', () => centerElement(helpPanelElement));
    ...
    $body.on('change input', `#${PANEL_ID} input, #${PANEL_ID} select`, function () {
      ...
      switch (id) {
        case 'st-mod-ss-scale':
          ...
          break;
        case 'st-mod-ss-format':
          currentSettings.imageFormat = $target.val();
          break;
        case 'st-mod-ss-quality':
          currentSettings.imageQuality = parseFloat($target.val()) || defaultSettings.imageQuality;
          break;
        case 'st-mod-ss-no-background':
          currentSettings.noBackground = $target.prop('checked');
          break;
        case 'st-mod-ss-custom-icon-enable':
          currentSettings.useCustomIcon = $target.prop('checked');
          break;
        case 'st-mod-ss-custom-icon-url':
          currentSettings.customIconUrl = $target.val();
          break;
      }

      $(`#${PANEL_ID} #st-mod-ss-quality`).prop(
        'disabled',
        !['jpeg', 'webp'].includes(settings.pluginSettings.imageFormat),
      );
...
async function customImageFetchFn(url) {
  ...
  captureLogger.info(`${logPrefix} Request received for URL:`, { url });
  if (!url || typeof url !== 'string') {
    captureLogger.warn(`${logPrefix} Invalid URL detected: URL is null or not a string. Skipping.`, { url });
    return false;
  }
  try {
    ...
    if (urlObj.pathname === '/' && urlObj.search === '' && urlObj.hash === '') {
      captureLogger.error(
        `${logPrefix} CRITICAL: URL points to the web root. This is likely an error from an empty 'src' attribute in an <img> tag. Skipping fetch.`,
        { url },
      );
      return false;
    }
  } catch (e) {
    captureLogger.error(`${logPrefix} Invalid URL format. Skipping.`, { url, error: e.message });
    return false;
  }
  if (url.startsWith('data:')) {
    captureLogger.info(`${logPrefix} URL is a data: URI. No fetch needed.`);
    return false;
  }
  if (IMAGE_DATA_MEMORY_CACHE.has(url)) {
    captureLogger.info(`${logPrefix} Found in L1 Memory Cache.`);
    return IMAGE_DATA_MEMORY_CACHE.get(url);
  }
  captureLogger.info(`${logPrefix} Not in L1. Checking L2 IndexedDB Cache...`);
  let dataUrl = await assetCacheManager.getImageData(url);
  if (dataUrl) {
    IMAGE_DATA_MEMORY_CACHE.set(url, dataUrl);
    captureLogger.info(`${logPrefix} Found in L2 IndexedDB. Loaded into L1.`);
    return dataUrl;
  }
  captureLogger.warn(`${logPrefix} Not in any cache. Fetching from network...`);
  try {
    const response = await fetch(url, { mode: 'cors' });
    if (!response.ok) {
      const errorData = { url, status: response.status, statusText: response.statusText };
      captureLogger.error(`${logPrefix} Network fetch FAILED: Non-2xx response.`, errorData);
    ...
    if (!contentType || !contentType.startsWith('image/')) {
      ...
      captureLogger.error(
        `${logPrefix} Network fetch FAILED: Response is not an image. This confirms an invalid image link.`,
        errorData,
      );
      return Promise.reject(`URL is not an image type: ${contentType}`);
    }
    captureLogger.info(`${logPrefix} Network fetch SUCCESS. Converting blob to Data URL...`);
    ...
    captureLogger.info(`${logPrefix} SUCCESS. Image fetched, converted, and cached.`);
    return dataUrl;
  } catch (err) {
    captureLogger.error(`${logPrefix} CRITICAL EXCEPTION during network fetch. Often a CORS issue.`, {
      url,
...
async function createUnitBackgroundAsync(scale) {
  ...
  if (currentSettings.noBackground) {
    captureLogger.info('"No Background" is enabled. Creating a transparent background canvas.');
  ...
  captureLogger.info('--- Background Image Processing Started ---', { scale });
  try {
    ...
    captureLogger.info('--- Background Image Processing Finished ---');
  }
...
async function captureLongScreenshot(elementsToCapture) {
  captureLogger.info('--- New Screenshot Process Started ---', {
  ...
  const timer =
    (label, start = performance.now()) =>
    () =>
      captureLogger.info(`⏱️ [Performance] ${label}: ${(performance.now() - start).toFixed(2)} ms`);
  ...
  if (currentSettings.imageFormat === 'webp' && (finalWidth > MAX_WEBP_DIMENSION || finalHeight > MAX_WEBP_DIMENSION)) {
    ...
    captureLogger.warn(
      `WebP dimension limit (${MAX_WEBP_DIMENSION}px) exceeded. Temporarily falling back to JPEG for this capture.`,
    );
...
function initLongScreenshotUI() {
  ...
  const startButton = $(
    `<div id="long_screenshot_start_button" class="menu_button">${getScreenshotIconHtml(
      'fa-solid fa-scroll',
    )}<span class="menu_button_text"> Long Screenshot</span></div>`,
  );
  ...
  startButton.on('click', () => {
    ...
    startButton.hide();
    const captureButton = $(
      `<div id="long_screenshot_capture_button" class="menu_button">${getScreenshotIconHtml()}<span class="menu_button_text"> Capture</span></div>`,
    );
    const cancelButton = $(
      '<div id="long_screenshot_cancel_button" class="menu_button"><i class="fa-solid fa-times"></i><span class="menu_button_text"> Cancel</span></div>',
    );
...
async function triggerMultiScreenshot(currentMessage, countBefore, countAfter, buttonElement) {
  ...
  captureLogger.info(
    `Triggering multi-message screenshot. Before: ${countBefore}, After: ${countAfter}. Total found: ${elementsToCapture.length}`,
  );
  // --- START: 新增的动画处理逻辑 ---
...
function addScreenshotButtonToMessage(messageElement) {
  if (
    !messageElement ||
    typeof messageElement.querySelector !== 'function' ||
    messageElement.querySelector(`.${config.buttonClass}`)
  )
    return;
  ...
  const handleClickOutside = event => {
    if (
      ...
      !multiCaptureMenu.contains(event.target) &&
      !screenshotButton.contains(event.target)
    ) {
      hideMenu();
    }
  };
...
function initFloatingScreenshotUI() {
  ...
  const styles = `
      <style id="${STYLE_ID}">
          ...
          #${FLOATING_UI_ID} button:disabled {
              opacity: 0.5;
              cursor: not-allowed;
          }
          #${FLOATING_UI_ID} button.active-selection {
              background-color: #ff9800;
              color: #000;
              font-weight: bold;
          }
          .message-selecting-mode #chat {
  ...
  const uiHtml = `
      <div id="${FLOATING_UI_ID}" style="display:none;">
  ...
  toggleButton.on('click', () => {
    ...
    if (panel.is(':visible')) {
      panel.removeClass('visible').fadeOut(200);
      resetCustomSelection(); // 关闭时重置状态
    } else {
      panel.fadeIn(200).addClass('visible');
    }
  });

  // --- 4. 绑定我们面板上按钮的事件 ---
  bindFloatingButtonEvents(); // 我们将在下一步实现这个函数
}

// 在 initFloatingScreenshotUI 函数后面添加这个新函数
...
// 在 bindFloatingButtonEvents 函数后面添加这些新函数
...
function updateFloatingButtonsUI() {
  ...
  if (customSelectionState === 'selecting_start') {
    startBtn.addClass('active-selection');
  } else if (customSelectionState === 'selecting_end') {
    endBtn.addClass('active-selection');
  }

  // 更新按钮文本
  startBtn.find('span').text(startMessageElement ? '重新设置起点' : '设置起点');
  endBtn.find('span').text(endMessageElement ? '重新设置终点' : '设置终点');
 ...
 * @param {HTMLElement} el1 - 起始或结束元素
 * @param {HTMLElement} el2 - 另一个起始或结束元素
 * @returns {HTMLElement[]} - 按文档顺序排列的元素数组
 */
...
async function initializePlugin() {
  try {
    ...
    captureLogger.info(
      `Resource cache pre-warmed. Loaded ${fontCount} fonts and ${imageCount} images. Took ${(
    ...
    initSettingsUI();
    initFloatingScreenshotUI(); // <--- 新增的UI初始化函数
